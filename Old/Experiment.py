...
if int(expSetup["Condition (Randomize)"])% 2==1:
    cond= 1
else:
    cond= 2
...
while not DorothyEnd: # for each of the trials
	trialEnd= False
	cond= 1				
	stimuliOn= False
	...
	while not stimuliOn: # repeats loop until gazebox is triggered within x seconds
		...
		tracker.status_msg('TRIAL E%dI%dD0' % (cond, item)) 
		...
		scr.draw_image('Dorothy/Dorothy'+ str(item)+ '.bmp')
	...
	while not trialEnd:
		...
		if 'right' in pressed: # terminate trial when mouse is clicked (temporary)
			goNext= True
			trialEnd= True
		if 'left' in pressed: # terminate trial when mouse is clicked (temporary)
			goNext= False
			trialEnd= True
